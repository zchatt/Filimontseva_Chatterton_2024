...
### linear mixed-effects model of iNM Area between diagnosis groups for SNV and LC ; log10(area) as % of CTR
# select and format data
data_table <- df_agg_merged[which(df_agg_merged$intra.extra == "iNM" & df_agg_merged$Age > 57 &
                                    df_agg_merged$ROI %in% c("SNV","LC","VTA")),]

# format measurements
dplot <- data_table %>% dplyr::count(intra.extra,Diagnosis,Brainbank_ID_recode, ROI, Brainregion,Age,Sex,PMD,log10_Area, sort = TRUE)
dplot <- dplot[!is.na(dplot$intra.extra),]
data_table <- dplot[complete.cases(dplot$Brainregion),]
colnames(data_table) <- make.names(colnames(data_table))

# select columns
columns_to_calculate <- c("log10_Area")

# group by ROI and Diagnosis, then calculate the mean for each group
df <- data_table
group_means <- df %>%
  group_by(ROI, Diagnosis) %>%
  summarise(across(all_of(columns_to_calculate), mean, na.rm = TRUE), .groups = 'drop')

# filter for CTR means
ctr_means <- group_means %>%
  filter(Diagnosis == "CTR")
...
# test correct prc have been calculated 
mean(df_with_pct_of_ctr$pct_of_ctr_log10_Area[df_with_pct_of_ctr$ROI == "SNV" & 
                                               df_with_pct_of_ctr$Diagnosis == "CTR"])

# assign to data_table
data_table <- df_with_pct_of_ctr

# regression between CTR and PD for each ROI
y_list <- c("pct_of_ctr_log10_Area")
res_contrasts <- list()
roi_uniq <- unique(data_table$ROI)
main_lab <- roi_uniq
for(z in 1:length(roi_uniq)){
...
# write summary to file
df <- apply(stat.summary,2,as.character)
#write.table(df, file="Area.iNM_Dx.SNVLCVTA_stat.summary.txt", sep="\t",row.names = F, quote = F)

# ## plot in single violin plot
# tmp <- aggregate(pct_of_ctr_log10_Area ~ ROI , data_table, mean)
# fact_lvls <- tmp[order(-tmp[,"pct_of_ctr_log10_Area"]),][,"ROI"]
# data_table[,"ROI"] <- factor(data_table[,"ROI"], levels = fact_lvls)

dplot <- data_table[data_table$Diagnosis != "CTR",]
g1 <- ggviolin(
  dplot, x = "Diagnosis", y = "pct_of_ctr_log10_Area",
  fill = "Diagnosis",
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size=10),
        plot.title = element_text(size = 10)) +
  ylab("NM Area % Controls") + xlab("") + ylim(60,160) +
  facet_wrap(~ROI,  nrow = 1) + theme(legend.key = element_blank(), strip.background = element_rect(colour="white", fill="white") ) 



arrange <- ggarrange(plotlist=list(g1), nrow=2, ncol=2, widths = c(2,2))
ggsave("Vln_iNM_area.SNV.VTA.LC_CTRPD.pdf", arrange,width = 8, height = 6)
#ggsave("Vln_iNM_n_per.mm2.ROI.pdf", arrange)
...
### linear mixed-effects model of iNM Density between diagnosis groups for SNV and LC ; n_per.mm2 % of CTR
# select and format data
data_table <- df_agg_merged[which(df_agg_merged$intra.extra == "iNM" & df_agg_merged$Age > 57 &
                                    df_agg_merged$ROI %in% c("SNV","LC","VTA")),]
...
# write summary to file
df <- apply(stat.summary,2,as.character)
#write.table(df, file="Density.iNM_Dx.SNVLCVTA_stat.summary.txt", sep="\t",row.names = F, quote = F)
...
dplot <- data_table[data_table$Diagnosis != "CTR",]
g1 <- ggviolin(
  dplot, x = "Diagnosis", y = "pct_of_ctr_n_per.mm2",
  fill = "Diagnosis",
  palette = Diagnosis_col,
  add = "mean_sd", size = 0.2) +
  theme(legend.position = "none") +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size=10),
        plot.title = element_text(size = 10)) +
  ylab("NM Density % Controls") + xlab("") + ylim(-50,350) +
  facet_wrap(~ROI,  nrow = 1) + theme(legend.key = element_blank(), strip.background = element_rect(colour="white", fill="white") ) 



arrange <- ggarrange(plotlist=list(g1), nrow=2, ncol=2, widths = c(2,2))
ggsave("Vln_iNM_density.SNV.VTA.LC_CTRPD.pdf", arrange,width = 8, height = 6)
#ggsave("Vln_iNM_n_per.mm2.ROI.pdf", arrange)
...
### linear mixed-effects model of Color Direction for SNV, LC and VTA; Direction of R,G,B as % of CTR
# select and format data
data_table <- df_agg_merged[which(df_agg_merged$intra.extra == "iNM" & df_agg_merged$Age > 57 &
                                    df_agg_merged$ROI %in% c("SNV","LC","VTA")),]

# format measurements
dplot <- data_table %>% dplyr::count(intra.extra,Diagnosis,Brainbank_ID_recode, ROI, Brainregion,Age,Sex,PMD,log10_Area,Direction.Red,Direction.Green,Direction.Blue, sort = TRUE)
dplot <- dplot[!is.na(dplot$intra.extra),]
data_table <- dplot[complete.cases(dplot$Brainregion),]
colnames(data_table) <- make.names(colnames(data_table))

# select columns
columns_to_calculate <- c("Direction.Red","Direction.Green","Direction.Blue")
...
# test correct prc have been calculated 
mean(df_with_pct_of_ctr$pct_of_ctr_Direction.Red[df_with_pct_of_ctr$ROI == "SNV" & 
                                                df_with_pct_of_ctr$Diagnosis == "CTR"])
...
# regression between CTR and PD for each ROI
y_list  <- c("pct_of_ctr_Direction.Red","pct_of_ctr_Direction.Green","pct_of_ctr_Direction.Blue")
res_contrasts <- list()
roi_uniq <- unique(data_table$ROI)
main_lab <- roi_uniq
for(z in 1:length(roi_uniq)){
  ...
  for(i in 1:length(y_list)){
    ...
    # Fit a linear mixed-effects model
    model <- lmer(formula(paste(eval(y_variable),"~",x_variable," + (1 | Age) + (1 | Sex) + (1 | PMD) + (1 | log10_Area)")), data = data_table2)
    ...
    if(all(stat.test$group1 == tmp$group1 & stat.test$group2 == tmp$group2)){
      ...
      stat.test$sd_group2 <- as.numeric(mapvalues(stat.test$group2,summary_df$x,summary_df$sd_y))
      stat.test$contrast <- roi_cont
      stat.test$contrast_2 <- y_variable
    }
    res_tmp[[i]] <- stat.test
  }
  # assign to list
  res_contrasts[[z]] <- do.call(rbind,res_tmp)
...
# write summary to file
df <- apply(stat.summary,2,as.character)
#write.table(df, file="Direction.RGB.iNM_Dx.SNVLC_stat.summary.txt", sep="\t",row.names = F, quote = F)
...
data_table_long <- data_table %>%
  pivot_longer(
    cols = c("pct_of_ctr_Direction.Red","pct_of_ctr_Direction.Green","pct_of_ctr_Direction.Blue"),  names_to = "Condition", values_to = "Value")



dplot <- data_table_long[data_table_long$Diagnosis != "CTR" & data_table_long$ROI == "LC" ,]
dplot$Condition <- gsub("pct_of_ctr_Direction.","",dplot$Condition)

g1 <- ggviolin(
  dplot, x = "Diagnosis", y = "Value",
  fill = "Condition",
  palette = RGB_col,
  add = "mean_sd", size = 0.2, alpha = 0.5) +
  theme(legend.position = "none") +
  geom_hline(yintercept=100, linetype="dashed", color = "grey") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size=10),
        plot.title = element_text(size = 10)) +
  ylab("Direction Color (% CTR's)") + xlab("") + 
  #ylim(50,160) +
  facet_wrap(~Condition,  nrow = 1,scales = "free") + theme(legend.key = element_blank(), strip.background = element_rect(colour="white", fill="white") ) 

dplot <- data_table_long[data_table_long$Diagnosis != "CTR" & data_table_long$ROI == "SNV" ,]
dplot$Condition <- gsub("pct_of_ctr_Direction.","",dplot$Condition)
...
dplot <- data_table_long[data_table_long$Diagnosis != "CTR" & data_table_long$ROI == "VTA" ,]
dplot$Condition <- gsub("pct_of_ctr_Direction.","",dplot$Condition)
...
arrange <- ggarrange(plotlist=list(g1,g2,g3), nrow=2, ncol=2, widths = c(2,2), labels = c("LC","SNV","VTA"))
ggsave("Vln_iNM_Col.Dir_prcctr.LC.SNV.VTA.pdf", arrange,width = 8, height = 6)
...
### linear mixed-effects model of Color Direction ratios for SNV, LC and VTA; Direction of R/ Direction of B
# select and format data
data_table <- df_agg_merged[which(df_agg_merged$intra.extra == "iNM" & df_agg_merged$Age > 57 &
                                    df_agg_merged$ROI %in% c("SNV","LC","VTA")),]
...
# caclulate R:B ratio
data_table$RB_ratio <- data_table$Direction.Red / data_table$Direction.Blue
data_table$RG_ratio <- data_table$Direction.Red / data_table$Direction.Green
data_table$BG_ratio <- data_table$Direction.Blue / data_table$Direction.Green


# regression between CTR and PD for each ROI
y_list  <- c("RB_ratio","RG_ratio","BG_ratio")
res_contrasts <- list()
roi_uniq <- unique(data_table$ROI)
main_lab <- roi_uniq
for(z in 1:length(roi_uniq)){
...
g1 <- ggboxplot(
  ...
  ylab("Direction Color (% CTR's)") + xlab("") + 
  ylim(50,160) +
  facet_wrap(~Condition,  nrow = 1) + theme(legend.key = element_blank(), strip.background = element_rect(colour="white", fill="white") ) 

arrange <- ggarrange(plotlist=list(g1), nrow=2, ncol=2, widths = c(2,2))
#ggsave("Bxp_iNM_Col.Dir_prcctr.LC.pdf", arrange,width = 8, height = 6)
...
##########################################################
## Part 6: Evaluate color variables in Controls by ROI ###
##########################################################
...
# ############################################################################################
# #### Part 7 - Classification of NM granules using Area and Color Directions
# ############################################################################################
# density function
...
# switch LC clusters to align with SNV
...
load(file="/Users/zacc/USyd/spatial_transcriptomics/analysis/geomx/geomx_oct2023/nm_data_280224.Rdata")
...
# select each cluster and calculate prc of each cluster
...
#ggsave("Vln_iNMcluster.prc.total.LC_v4.pdf", arrange,width = 8, height = 6)
#ggsave("Vln_iNMcluster.prc.total.SNV_v4.pdf", arrange,width = 8, height = 6)
